...
class TestEntityTimeRange:
    def test_init(self):
        ...
        assert r.begin == datetime.datetime(year=2019, month=2, day=8, hour=12, minute=27, second=20, tzinfo=tzutc())
        assert r.end == datetime.datetime(year=2019, month=2, day=8, hour=13, minute=27, second=20, tzinfo=tzutc())
    ...
    def test_contains(self):
        ...
        with pytest.raises(TypeError):
            r = TimeRange("2019-02-08T12:27:20Z/2019-02-08T13:27:20Z")
            assert datetime.datetime(year=2019, month=2, day=8, hour=12, minute=28) not in r
    ...
    def test_range_open(self):
        ...
        assert datetime.datetime(year=2019, month=12, day=31, hour=23, minute=59) in open_start
        ...
        assert datetime.datetime(year=2019, month=12, day=31, hour=23, minute=59, second=59) in open_end
        assert datetime.datetime(year=2019, month=12, day=31, hour=23, minute=59, second=58) not in open_end
        ...
        assert datetime.datetime(year=2019, month=12, day=31, hour=23, minute=59, second=59) == next(weeks)
        assert datetime.datetime(year=2020, month=1, day=7, hour=23, minute=59, second=59) == next(weeks)
    ...
    def test_str(self):
        ...
        assert str(r) == '<TimeRange begin="2019-02-08 12:27:20" end="2019-03-01 13:27:20">'
...
class TestEntityAttributeV2:
    ...
    def test_init_with_kwargs(self):
        ...
        assert "AttributeV2(id=0, value=123456, nested_in=[1], overlaps_with=[2], extras=None)" == repr(attr)
    ...
    def test_repr(self):
        ...
        assert "AttributeV2(id=1, value=123456, nested_in=[1, 2], overlaps_with=[3], extras=None)" == repr(attr)


class TestConverter:
    def test_converter(self):
        assert convert("2106-12-31T12:30", datetime.date) == datetime.date(2106, 12, 31)
        assert convert("2106-12-31T12:30", datetime.datetime) == datetime.datetime(2106, 12, 31, 12, 30, 00)
        ...
        assert convert("PT1H5M6S", datetime.timedelta) == datetime.timedelta(hours=1, minutes=5, seconds=6)
    ...
    def test_datetime(self):
        assert entities.to_datetime("2106-12-31T12:30") == datetime.datetime(2106, 12, 31, 12, 30, 00)
        assert entities.to_datetime(datetime.date(2106, 12, 31)) == datetime.datetime(2106, 12, 31, 00, 00, 00)

        now = datetime.datetime.now()
        assert entities.to_datetime(datetime.time(12, 30)) == datetime.datetime(now.year, now.month, now.day, 12, 30, 00)
        assert entities.to_datetime(["2106-12-31T12:30"]) == datetime.datetime(2106, 12, 31, 12, 30, 00)
        assert entities.to_datetime(["2106-12-31T12:30", "2116-12-31T12:30"]) == datetime.datetime(2106, 12, 31, 12, 30, 00)
        assert entities.to_datetime(["--12-31"]) == datetime.datetime(datetime.datetime.now().year, 12, 31, 00, 00)
        assert entities.to_datetime(["--12-31T12:30"]) == datetime.datetime(datetime.datetime.now().year, 12, 31, 12, 30)
        assert entities.to_datetime([]), datetime.datetime.min

    def test_date(self):
        assert entities.to_date("2106-12-31T12:30") == datetime.date(2106, 12, 31)
        assert entities.to_date(datetime.datetime(2106, 12, 31, 12, 30, 00)) == datetime.date(2106, 12, 31)
        assert entities.to_date(["2106-12-31T12:30"]) == datetime.date(2106, 12, 31)
        assert entities.to_date(["2106-12-31T12:30", "2116-12-31T12:30"]) == datetime.date(2106, 12, 31)
        assert entities.to_date(["--12-31"]) == datetime.date(datetime.datetime.now().year, 12, 31)
        assert entities.to_date(["--12-31T12:30"]) == datetime.date(datetime.datetime.now().year, 12, 31)
        assert entities.to_date([]) == datetime.date.min

    def test_time(self):
        assert entities.to_time("2106-12-31T12:30") == datetime.time(12, 30)
        assert entities.to_time(datetime.datetime(2106, 12, 31, 12, 30, 00)) == datetime.time(12, 30)
        assert entities.to_time(["2106-12-31T12:30"]) == datetime.time(12, 30)
        assert entities.to_time(["2106-12-31T12:30", "2116-12-31T15:30"]) == datetime.time(12, 30)
